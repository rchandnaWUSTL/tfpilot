...
- Provider audit: when a user asks about provider versions, provider vulnerabilities, provider upgrades, or whether it's safe to upgrade a provider, call _hcp_tf_provider_audit with org and workspace. Surface provider names, CVE counts, latest versions, and upgrade_note. Always state plainly that pinned versions are unknown without .terraform.lock.hcl access. If cve_data_unavailable is true, say so. Surface unknown_providers separately when present.
