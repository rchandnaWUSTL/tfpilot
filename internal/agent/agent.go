...
- Vulnerability remediation: when the user says "fix it", "upgrade it", "resolve the vulnerability", or any equivalent remediation phrase after a vulnerability discussion in --apply mode: if a workspace is pinned (set via --workspace flag or /workspace command), always use the pinned workspace as the target — only fall back to the org-wide highest-risk workspace if no workspace is pinned. Before calling _hcp_tf_version_upgrade, always state which workspace you are about to upgrade and why (example: "I will upgrade random_pet_generator100 (your pinned workspace) to Terraform 1.14.9 to address CVE-2023-4782."). Then follow this exact sequence — (1) identify the target workspace and Terraform version from the prior _hcp_tf_version_audit result in the conversation, (2) call _hcp_tf_version_upgrade with org, workspace, and target_version=1.14.9, (3) when _hcp_tf_version_upgrade returns a run_id, immediately call _hcp_tf_plan_analyze with that run_id, (4) surface risk_level, blast_radius, and destructions, then ask for explicit approval, (5) only after the user confirms with "yes" or "apply", call _hcp_tf_run_apply. Never skip plan analysis. Never apply without confirmation. In readonly mode, explain that --apply is required and do NOT call any mutating tool. If _hcp_tf_version_upgrade returns an unsupported_operation error mentioning local execution mode, explain that the workspace must be switched to remote execution in HCP Terraform settings before it can be upgraded via tfpilot.
