...
## v2.2 — Console-to-Code Import
- Bring resources that were click-opsed in the cloud console under Terraform management without the user ever typing an ARN. The wedge is the inventory-diff flow: "what's in my AWS account that isn't in any of my workspaces?" → ranked list → one keystroke imports them all.
- _hcp_tf_cloud_inventory tool: scans a configured cloud account (AWS first; GCP/Azure later) for resources in a region + resource-type allowlist covering the ~15 highest-value AWS types — EC2 instances, S3 buckets, RDS, IAM roles/policies, Lambda, ALB/NLB, VPC/subnet/SG, ECR, ECS, EKS, SQS, SNS, DynamoDB, CloudWatch alarms, Route53 zones. Read-only. Rides the standard AWS credential chain (env vars, shared config, IRSA) so the user re-uses the credentials `terraform plan` already needs locally.
- _hcp_tf_import_diff tool: diffs cloud inventory against the bound workspace's state and returns unmanaged resources ranked by import-worthiness — tagged `prod`/`production` first, then stateful resources (RDS, DynamoDB, S3 with objects), then everything else. Read-only.
- _hcp_tf_resource_introspect tool: given a cloud resource identifier, calls the relevant Describe* API and returns a normalized attribute bag the config generator can consume. Read-only.
- _hcp_tf_import_generate tool: emits HCL for the target resource (reusing the v0.6 config-generation pipeline) plus a Terraform 1.5+ `import { to = ..., id = ... }` block, writes both to cwd, and runs a speculative plan against the workspace. Uploads a speculative configuration version that is discarded after analysis — same pattern as the v1.6 upgrade preview. Confirms the diff is "import only, 0 changes / 0 destroys" before offering to apply; any non-import change surfaces as a vaultYellow warning and blocks one-keystroke apply.
- _hcp_tf_import_apply tool: promotes the validated import to a real configuration version and triggers a run through the existing v0.5 approval gate. Mutating — requires --apply.
- /import slash command, two shapes:
  - `/import` with no arguments → runs the inventory diff and renders the ranked list with a single approval prompt: `Import these N resources? [y/n/select]`. `select` opens an interactive multi-select for cherry-picking. `y` walks the list, chaining introspect → generate → speculative plan → apply per resource with one approval gate per resource. Destructions > 0 in the speculative plan (which should never happen for a clean import) still triggers the v0.5 second-confirm.
  - `/import <natural language>` → e.g. `/import the S3 bucket called acme-uploads` or `/import that load balancer I just made in us-east-1`. Agent path: resolves the description to a concrete resource via cloud_inventory + fuzzy match, then runs the same chain for the single resource.
- System prompt rule: "I clicked together X in the console", "bring X under Terraform", "import X", "what's not in Terraform yet" route to the import flow. The agent never asks the user for an ARN — it asks the cloud API.
- Lowest-friction default: when the speculative plan confirms `import only, 0 changes`, the apply gate auto-promotes to a single-keystroke `y` confirmation. Any deviation (proposed updates, replacements, destroys) downgrades to the full v0.5 approval prose with the diff inline.
- Graceful degradation: missing AWS credentials surfaces a clear `auth_error` naming the env var or `~/.aws/config` profile expected; cloud API throttling returns `retryable: true`; resources outside the allowlist return `unsupported_resource_type` with the resource type the user named so the next release can add it.
- Readonly mode supports cloud_inventory, import_diff, and resource_introspect so a user can explore "what would tfpilot import for me" before flipping --apply on; import_generate's speculative upload and import_apply both require --apply.
- Demo target: `tfpilot --org=sarah-test-org --workspace=prod-k8s-apps --apply` → `/import` → ranked list of unmanaged resources in the bound AWS account → one `y` → workspace state now manages them, audit log records every step.
